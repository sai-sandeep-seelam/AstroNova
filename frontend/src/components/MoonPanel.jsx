...
function MoonPhaseSVG({ illumination = 0, phase = 'Full Moon' }) {
  ...
  return (
    <svg width={80} height={80} viewBox="0 0 80 80" style={{ filter: 'drop-shadow(0 0 12px rgba(251,191,36,0.4))' }}>
      {/* dark sphere */}
      <circle cx={40} cy={40} r={r} fill="#1a1a2e" stroke="rgba(251,191,36,0.2)" strokeWidth={1} />
      {/* illuminated portion */}
      {isFull ? (
      ...
      {/* terminator ring */}
      <circle cx={40} cy={40} r={r} fill="none" stroke="rgba(251,191,36,0.35)" strokeWidth={1.5} />
    </svg>
  );
}
...
const MoonPanel = () => {
  ...
  const altitudeDeg   = parseFloat(moonCells?.position?.horizontal?.altitude?.degrees ?? moon?.altitude ?? 0).toFixed(1);
  const azimuthDeg    = parseFloat(moonCells?.position?.horizontal?.azimuth?.degrees  ?? moon?.azimuth  ?? 0).toFixed(1);

  // ── Bodies / planets ──────────────────────────────────────────
  const planetRows = bodiesData?.data?.table?.rows?.filter(r => {
...
const MoonStat = ({ label, value }) => (
  <div>
    <div style={{ fontSize: 9, color: 'rgba(255,255,255,0.35)', letterSpacing: '0.08em', marginBottom: 2 }}>{label}</div>
    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}>
          ...
          {planet.visible && (
            <span style={{
              ...
              borderRadius: 999, padding: '1px 5px', letterSpacing: '0.04em',
            }}>VISIBLE</span>
          )}
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          {planet.magnitude !== '—' && (
            <PlanetStat label="MAG" value={planet.magnitude} />
          )}
        </div>
      </div>
